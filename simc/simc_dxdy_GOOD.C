...
void simc_dxdy(){
	...
	W_mean = lookup_simc_cut(kine, sbsfieldscale, run_target, "W");
	W_sigma = lookup_simc_cut(kine, sbsfieldscale, run_target, "W_sigma");
	...
	rootfile_dir = "/lustre19/expphy/volatile/halla/sbs/jboyd/simulation/out_dir/MC_REPLAY_OUT_DIR/simc";
	...
	outfilename = Form("rootfiles/simc_SBS%i_%s_mag%i_%suA_dxdy_trPfact_100_19_09_2023.root", kine, run_target.Data(), sbsfieldscale, I_beam_str.Data());
	...
	//INITIALIZE HISTOGRAMS
		...
		h_simc_dx_cut = new TH1D("h_simc_dx_cut",Form("dx (Basic CUTS) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);
		...
		h_simc_dy_cut = new TH1D("h_simc_dy_cut",Form("dy (Basic Cuts) - SBS%i = %i%%, %s; y_{HCal} - y_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 250, -1.25, 1.25);  
		...
		h_simc_dxdy_cut = new TH2D("h_simc_dxdy_cut", Form("Hadron Spot(s) on HCal (Basic cuts) - SBS%i = %i%%, %s;y_{HCal}-y_{expect} (m); x_{HCal}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data()), 300, -1.5, 1.5, 500, -2.5, 2.5 );
		h_simc_dxdy_ncut = new TH2D("h_simc_dxdy_ncut", Form("Hadron Spot(s) on HCal (n cut) - SBS%i = %i%%, %s;y_{HCal}-y_{expect} (m); x_{HCal}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data()), 250, -1.25, 1.25, 500, -2.5, 2.5 );
		h_simc_dxdy_pcut = new TH2D("h_simc_dxdy_pcut", Form("Hadron Spot(s) on HCal (p cut) - SBS%i = %i%%, %s;y_{HCal}-y_{expect} (m); x_{HCal}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data()), 250, -1.25, 1.25, 500, -2.5, 2.5 );
	...
	// Set BRANCH ADDRESSES
		...
		master_cut_vec = {
			...
			"bb.tr.n==1",
			Form("bb.tr.p[0]>%f", 1.00*lookup_parsed_cut(run_target.Data(), kine, sbsfieldscale, "SH_PS_mean") ),
			// Form("((abs(((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))))-%f)<%f", lookup_simc_cut(kine, sbsfieldscale, run_target, "Ep"), 3.0*lookup_simc_cut(kine, sbsfieldscale, run_target, "Ep_sigma")),
	...
	//---------"dxdy" method
		...
		//FIDUCIAL Cut
			...
			if( fiducial_cut ){
				...
				if( sbsfieldscale != 0 ){
					dx_p = lookup_simc_dxdy(kine, sbsfieldscale, run_target, "dx_p");
					dx_p_sigma = dx_p_scale*lookup_simc_dxdy(kine, sbsfieldscale, run_target, "dx_p_sigma");
					dy_p = lookup_simc_dxdy(kine, sbsfieldscale, run_target, "dy");
					dy_p_sigma = lookup_simc_dxdy(kine, sbsfieldscale, run_target, "dy_sigma");
					dx_n = lookup_simc_dxdy(kine, sbsfieldscale, run_target, "dx_n");
					dx_n_sigma = dx_n_scale*lookup_simc_dxdy(kine, sbsfieldscale, run_target, "dx_n_sigma");
					dy_n = lookup_simc_dxdy(kine, sbsfieldscale, run_target, "dy");
					dy_n_sigma = lookup_simc_dxdy(kine, sbsfieldscale, run_target, "dy_sigma");
