...
TH1D *h_MC_dx, *h_MC_dx_cut, *h_MC_dx_wcut, *h_MC_dx_fcut, *h_MC_dx_wcut_fcut;
TH1D *h_MC_dx_p, *h_MC_dx_n, *h_MC_dx_p_wcut, *h_MC_dx_n_wcut, *h_MC_dx_p_fcut, *h_MC_dx_n_fcut;
TH1D *h_MC_dx_p_wcut_fcut, *h_MC_dx_n_wcut_fcut;
TH1D *h_MC_dy, *h_MC_dy_cut, *h_MC_dy_wcut;
TH2D *h_MC_dxdy, *h_MC_dxdy_cut, *h_MC_dxdy_wcut, *h_MC_dxdy_ncut, *h_MC_dxdy_pcut, *h_MC_dxdy_fcut;
...
// double I_beam_uA = 12.0;
	...
	if( kine == 8 ){
		HCal_height = 0.005; // Modified to calibrate peak centers for MC: 0.168695, 0.25235289
	}
	if( kine == 9 ){
		HCal_height = 0.0;
	}
	...
	magmod = "0308";
	outfile = new TFile(Form("rootfiles/MC_SBS%i_%s_mag%imod%s_%suA_dxdy_15_08_2023.root", kine, run_target.Data(), sbsfieldscale, magmod.Data(), I_beam_str.Data()), "RECREATE");	
...
//INITIALIZE HISTOGRAMS
	...
	h_MC_dx = new TH1D("h_MC_dx",Form("dx (NO CUTS) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);
	h_MC_dx_p = new TH1D("h_MC_dx_p",Form("dx - proton only - (NO CUTS) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);
	h_MC_dx_n = new TH1D("h_MC_dx_n",Form("dx - neutron only - (NO CUTS) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);	
	h_MC_dx_p_wcut = new TH1D("h_MC_dx_p_wcut",Form("dx - proton only - (W Cuts) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);
	h_MC_dx_n_wcut = new TH1D("h_MC_dx_n_wcut",Form("dx - neutron only - (W Cuts) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);	
	h_MC_dx_p_fcut = new TH1D("h_MC_dx_p_fcut",Form("dx - proton only - (F Cut) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);
	h_MC_dx_n_fcut = new TH1D("h_MC_dx_n_fcut",Form("dx - neutron only - (F Cut) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);	
	h_MC_dx_p_wcut_fcut = new TH1D("h_MC_dx_p_wcut_fcut",Form("dx - proton only - (W & F Cuts) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);
	h_MC_dx_n_wcut_fcut = new TH1D("h_MC_dx_n_wcut_fcut",Form("dx - neutron only - (W & F Cuts) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);	

	h_MC_dx_cut = new TH1D("h_MC_dx_cut",Form("dx (Basic CUTS) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);
	h_MC_dx_wcut = new TH1D("h_MC_dx_wcut",Form("dx (W cut) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);
	h_MC_dx_fcut = new TH1D("h_MC_dx_fcut",Form("dx (f cut) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);
	h_MC_dx_wcut_fcut = new TH1D("h_MC_dx_wcut_fcut",Form("dx (W & f cut) - SBS%i = %i%%, %s; x_{HCal} - x_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);
	h_MC_dy = new TH1D("h_MC_dy",Form("dy (NO CUTS) - SBS%i = %i%%, %s; y_{HCal} - y_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 500, -2.5, 2.5);
	h_MC_dy_cut = new TH1D("h_MC_dy_cut",Form("dy (Basic Cuts) - SBS%i = %i%%, %s; y_{HCal} - y_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 250, -1.25, 1.25);  
	h_MC_dy_wcut = new TH1D("h_MC_dy_wcut",Form("dy (W Cuts) - SBS%i = %i%%, %s; y_{HCal} - y_{exp} (m);", kine, sbsfieldscale, run_target.Data()), 250, -1.25, 1.25);  

	h_MC_dxdy = new TH2D("h_MC_dxdy", Form("Hadron Spot(s) on HCal (NO CUTS) - SBS%i = %i%%, %s;y_{HCal}-y_{expect} (m); x_{HCal}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data()), 300, -1.5, 1.5, 500, -2.5, 2.5 );
	h_MC_dxdy_wcut = new TH2D("h_MC_dxdy_wcut", Form("Hadron Spot(s) on HCal (W cut) - SBS%i = %i%%, %s;y_{HCal}-y_{expect} (m); x_{HCal}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data()), 300, -1.5, 1.5, 500, -2.5, 2.5 );
	h_MC_dxdy_cut = new TH2D("h_MC_dxdy_cut", Form("Hadron Spot(s) on HCal (Basic cuts) - SBS%i = %i%%, %s;y_{HCal}-y_{expect} (m); x_{HCal}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data()), 300, -1.5, 1.5, 500, -2.5, 2.5 );
	h_MC_dxdy_ncut = new TH2D("h_MC_dxdy_ncut", Form("Hadron Spot(s) on HCal (n cut) - SBS%i = %i%%, %s;y_{HCal}-y_{expect} (m); x_{HCal}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data()), 250, -1.25, 1.25, 500, -2.5, 2.5 );
	h_MC_dxdy_pcut = new TH2D("h_MC_dxdy_pcut", Form("Hadron Spot(s) on HCal (p cut) - SBS%i = %i%%, %s;y_{HCal}-y_{expect} (m); x_{HCal}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data()), 250, -1.25, 1.25, 500, -2.5, 2.5 );
	h_MC_dxdy_fcut = new TH2D("h_MC_dxdy_fcut", Form("Hadron Spot(s) on HCal (f cut) - SBS%i = %i%%, %s;y_{HCal}-y_{expect} (m); x_{HCal}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data()), 250, -1.25, 1.25, 500, -2.5, 2.5 );
	...
	if( kine == 4 ){
		...
		// TC->Add(infile.Data());
		TC->Add(Form("%s/replayed_jb_gmn_SBS4_LD2_mag50mod68_175uA_elas_100k_TEST_07_17_job0.root", rootfile_dir.Data()));	
	}
	...
	if( kine == 8 ){
		...
		infile = "/lustre19/expphy/volatile/halla/sbs/jboyd/simulation/out_dir/MC_REPLAY_OUT_DIR/replayed_digitized_jb_FARM_gmn_SBS8_LD2_mag70port0308_500uA_elas_voffsetFix_100k_14_08_2023_job*.root";
...
//---------"dxdy" method
	...
	//Resolve the hadron spots without cuts
		h_MC_dx->Fill( dx, pn_weight);
		//Selectively fill for proton or neutron:
		if( is_p ){
			h_MC_dx_p->Fill( dx, pn_weight );
		}
		if( is_n ){
			h_MC_dx_n->Fill( dx, pn_weight );
		}

		h_MC_dy->Fill( dy, pn_weight);
		h_MC_dxdy->Fill( dy, dx, pn_weight );
		h_xy->Fill( hcal_y, hcal_x );

	// Preliminary HCal projections with single cut on W
		if( fabs(W - W_mean) < W_sigma ){
			h_MC_dx_wcut->Fill( dx, pn_weight );
			h_MC_dy_wcut->Fill ( dy, pn_weight );
			h_MC_dxdy_wcut->Fill( dy, dx, pn_weight );

			//Histogram by hadron
			if( is_p ){
				h_MC_dx_p_wcut->Fill(dx, pn_weight);
			}
			if( is_n ){
				h_MC_dx_n_wcut->Fill(dx, pn_weight);
			}
		}
	...
	//FIDUCIAL Cut
		...
		//Fill respective histograms for these checks.
			if( HCal_on && is_n && apply_fcut ) h_MC_dxdy_ncut->Fill( dy, dx );
			if( HCal_on && is_p && apply_fcut ) h_MC_dxdy_pcut->Fill( dy, dx );
		...
		//----------neutron
			if( HCal_on && is_n && apply_fcut ){
				...
				// if( (hcal_x - dx_pn_max )>hcal_x_fmin ){
					h_MC_dxdy_fcut->Fill( dy, dx, pn_weight );
					h_MC_dx_fcut->Fill( dx, pn_weight );
					h_W_fcut->Fill( W, pn_weight );
					h_xy_fcut->Fill( hcal_y, hcal_x, pn_weight );
					h_xy_cut_n->Fill( hcal_y, hcal_x );
					h_MC_dx_n_fcut->Fill(dx, pn_weight);
					if( fabs(W - W_mean) < W_sigma ){
						h_MC_dx_wcut_fcut->Fill(dx, pn_weight);
						h_MC_dx_n_wcut_fcut->Fill(dx, pn_weight);
					}
					elastic_yield++;
				// }
			}		
		//----------proton
				...
				// if( (hcal_x + dx_pn_max)<hcal_x_fmax ){
					...
					h_MC_dx_p_fcut->Fill(dx, pn_weight);
					if( fabs(W - W_mean) < W_sigma ){
						h_MC_dx_wcut_fcut->Fill(dx, pn_weight);
						h_MC_dx_p_wcut_fcut->Fill(dx, pn_weight);
					}
					elastic_yield++;
				// }
			}
...
//-------------
	...
	hin_dx = static_cast<TH1D*>(infile->Get("h_MC_dx"));
	hin_dx_fcut = static_cast<TH1D*>(infile->Get("h_MC_dx_fcut"));
	hin_dx_wcut_fcut = static_cast<TH1D*>(infile->Get("h_MC_dx_wcut_fcut"));
	hin_dxdy = static_cast<TH2D*>(infile->Get("h_MC_dxdy"));
	hin_dxdy_fcut = static_cast<TH2D*>(infile->Get("h_MC_dxdy_fcut"));
	hin_dxdy_wcut = static_cast<TH2D*>(infile->Get("h_MC_dxdy_wcut"));
	...
	if( calibrate ){
    	TFile *infile = new TFile(outfile->GetName(), "READ");
    	TH1D *hin_dx_wcut = static_cast<TH1D*>(infile->Get("h_MC_dx_wcut"));
    	TH1D *hin_dy_wcut = static_cast<TH1D*>(infile->Get("h_MC_dy_wcut"));
