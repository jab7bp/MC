...
void MC_calibrate(){
	...
	if( !calc_W ){
		outfile = new TFile(Form("rootfiles/MC_SBS%i_%s_mag%i_prime_calibration.root", kine, run_target.Data(), sbsfieldscale), "RECREATE");
	}
	if( calc_W ){
		outfile = new TFile(Form("rootfiles/MC_SBS%i_%s_mag%i_full_calibration.root", kine, run_target.Data(), sbsfieldscale), "RECREATE");
	}		


	//Define Histograms:
	h_Ep = new TH1D("h_Ep", Form("E/p (E/p > 0) - SBS%i = %i%%, %s, %s", kine, sbsfieldscale, run_target.Data(), runs_string.Data()), 200, 0, 2);
	h_PS = new TH1D("h_PS", Form("Pre-Shower Clus. E (PS > 0) - SBS%i = %i%%, %s, %s; GeV", kine, sbsfieldscale, run_target.Data(), runs_string.Data()), 300, 0, 3);
	h_HCal_e = new TH1D("h_HCal_e", Form("HCal Clus. E (HCal_E > 0) - SBS%i = %i%%, %s, %s; GeV", kine, sbsfieldscale, run_target.Data(), runs_string.Data()), 200, 0, 0.4);
	h_SHPS = new TH1D("h_SHPS", Form("SH + PS Clus. E (SH+PS > 0) - SBS%i = %i%%, %s, %s; GeV", kine, sbsfieldscale, run_target.Data(), runs_string.Data()), 500, 0, 5);
	h_W = new TH1D( "h_W", Form("Invariant Mass, W (No Cuts) - SBS%i = %i%%, %s, %s; GeV", kine, sbsfieldscale, run_target.Data(), runs_string.Data()), 200, 0.0, 2.0 );
	h_Wrecon = new TH1D( "h_Wrecon", Form("Recon. Invariant Mass, W  - SBS%i = %i%%, %s, %s; GeV", kine, sbsfieldscale, run_target.Data(), runs_string.Data()), 300, 0.0, 3.0 );
	h_W2 = new TH1D( "h_W2", Form("Invariant Mass, W^{2} - SBS%i = %i%%, %s, %s; GeV", kine, sbsfieldscale, run_target.Data(), runs_string.Data()), 400, 0.0, 4.0 );
	h_W2recon = new TH1D( "h_W2recon", Form("Reconstructed W^{2}s - SBS%i = %i%%, %s, %s; GeV", kine, sbsfieldscale, run_target.Data(), runs_string.Data()), 400, 0.0, 4.0 );
	h_vert = new TH1D( "h_vert", "Vertex Position; m", 200, -.1, .1 );	

	if( plot_dxdy ){
		h_dxdy_wcut = new TH2D("h_dxdy_wcut",Form("HCal dxdy (W cuts) - SBS%i = %i%%, %s, %s;y_{HCAL}-y_{expect} (m); x_{HCAL}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data(), runs_string.Data()), 400, -2, 2, 400, -2.0, 2.0 );
		h_dxdy_all = new TH2D("h_dxdy_all",Form("HCal dxdy (NO CUTS) - SBS%i = %i%%, %s, %s;y_{HCAL}-y_{expect} (m); x_{HCAL}-x_{expect} (m)", kine, sbsfieldscale, run_target.Data(), runs_string.Data()),400,-2,2,400,-2,2);
		h_dx = new TH1D( "h_dx", Form("HCal dx - SBS%i = %i%%, %s, %s; m", kine, sbsfieldscale, run_target.Data(), runs_string.Data()), 400, -2.0, 2.0 );
		h_dy = new TH1D( "h_dy", Form("HCal dy - SBS%i = %i%%, %s, %s; m", kine, sbsfieldscale, run_target.Data(), runs_string.Data()), 400, -2, 2 );
		h_Y = new TH1D( "h_Y", Form("HCal Y - SBS%i = %i%%, %s, %s; m", kine, sbsfieldscale, run_target.Data(), runs_string.Data()), 400, -2, 2 );
	}

	HCal_dist = lookup_HCal_dist_by_kine(kine); 	//Distace from HCal face to target chamber
	HCal_theta = lookup_HCal_angle_by_kine(kine, "rad");		//Theta angle for HCal from downstream beamline
	cout << endl << "-----------------------------------" << endl;
	...
	if( single_run ){
		...
		// TC->Add(Form("%s/replayed_jb_gmn_SBS9_LH2_mag70mod0815_sbsAng220_1500uA_elas_100k_09_08_2023_job_*.root", rootfile_dir.Data()));
		TC->Add("/lustre19/expphy/volatile/halla/sbs/jboyd/simulation/out_dir/MC_REPLAY_OUT_DIR/replayed_digitized_jb_FARM_gmn_SBS8_LD2_mag70port0308_500uA_elas_voffsetFix_100k_14_08_2023_job*");
	...
	if( calc_W ){
		master_cut_vec = {
			...
			Form("((abs(((bb.sh.e+bb.ps.e)/(bb.tr.p[0]))))-%f)<%f", lookup_MC_cut(kine, sbsfieldscale, run_target, "Ep"), 2.0*lookup_MC_cut(kine, sbsfieldscale, run_target, "Ep_sigma")),
...
//---------------- dxdy -----------------------	
	if( plot_dxdy ){
		...
		TH1D *h_dx_wcut = hin_dxdy_wcut->ProjectionY();
		TH1D *h_dy_wcut = hin_dxdy_wcut->ProjectionX();
		...
		tf_dx_wcut_p->SetParLimits(0, 0.9*h_dx_wcut->GetMaximum(), h_dx_wcut->GetMaximum());
		tf_dx_wcut_p->SetParLimits(1, dx_p_min, dx_p_max);
		tf_dx_wcut_p->SetParLimits(2, 0.01, dx_p_sigma_max);
		h_dx_wcut->Fit("tf_dx_wcut_p", "R+");
		...
		tf_dx_wcut_n->SetParLimits(0, 0.9*h_dx_wcut->GetMaximum(), h_dx_wcut->GetMaximum());
		tf_dx_wcut_n->SetParLimits(1, dx_n_min, dx_n_max);
		tf_dx_wcut_n->SetParLimits(2, 0.01, dx_n_sigma_max);
		h_dx_wcut->Fit("tf_dx_wcut_n", "R+");
		...
		tf_dy_wcut->SetParLimits(0, 0.9*h_dy_wcut->GetMaximum(), h_dy_wcut->GetMaximum());
		tf_dy_wcut->SetParLimits(1, dy_min, dy_max);
		tf_dy_wcut->SetParLimits(2, 0.01, dy_sigma_max);
		h_dy_wcut->Fit("tf_dy_wcut", "R+");
...
cout << "------------------------------------------------------------------"<< endl;
	...
	if( calc_W ){
		cout << "{ kine, sbsfieldscale, PS, SH_PS_center, SH_PS_sigma, HCal, Ep, Ep_sigma, W2, W2_sigma, W, W_sigma }" << endl << endl;
		cout << "{";
		// cout << "SBS" << kine << ", " << lookup_MC_cut(kine, sbsfieldscale, run_target, "PS_clus_e_cut") << ", ";
		// cout << lookup_MC_cut(kine, sbsfieldscale, run_target, "SH_PSmean") << ", " << lookup_MC_cut(kine, sbsfieldscale, run_target, "SH_PS_sigma") << ", ";
		// cout << lookup_MC_cut(kine, sbsfieldscale, run_target, "HCal_clus_e_cut") << ", " << lookup_MC_cut(kine, sbsfieldscale, run_target, "Ep") << ", ";
		// cout << lookup_MC_cut(kine, sbsfieldscale, run_target, "Ep_sigma") << ", " << W2_fit_center <<", " << W2_fit_sigma << ", ";
